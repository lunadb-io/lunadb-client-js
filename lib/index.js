...
export default class LunaDBClient {
  ...
  async v0betaCreateDocument(key) {
    return await this.request("/api/v0beta/documents", "POST", { key: key });
  }

  async v0betaDeleteDocument(key) {
    return await this.request("/api/v0beta/documents/" + key, "DELETE");
  }

  async v0betaLoadDocument(key) {
    return await this.request("/api/v0beta/documents/" + key, "GET");
  }
  ...
  async v0betaSyncDocument(key, last_synced, deltaset) {
    ...
    return await this.request("/api/v0beta/documents/" + key, "PATCH", body);
  }
